...
function monolougueImpl () {
  const keyDelimiter = "|";
  const bindingsResolver = {
    cache: {},
    regex: {},
    ...
    compare: function compare( binding, topic ) {
      ...
      let rgx;
      let prevSegment;
      let result = ( this.cache[ topic + keyDelimiter + binding ] );
      // result is cached?
      if ( result === true ) {
        return result;
      }
    ...
    purge: function( options ) {
      const self = this;
      const matchPredicate = function( val, key ) {
        const split = key.split( keyDelimiter );
        const topic = split[ 0 ];
        const binding = split[ 1 ];
  ...
  const SubscriptionDefinition = function( topic, callback, emitter ) {
    this.topic = topic;
  ...
  const ConsecutiveDistinctPredicate = function() {
    let previous;
  ...
  const DistinctPredicate = function DistinctPredicateFactory() {
    const previous = [];
    return function DistinctPredicate( data ) {
      const isDistinct = !_.some( previous, function( p ) {
  ...
  SubscriptionDefinition.prototype = {
    ...
    disposeAfter: function disposeAfter( maxCalls ) {
      ...
      const dispose = _.after( maxCalls, this.unsubscribe.bind( this ) );
    ...
    throttle: function throttle( milliseconds ) {
      ...
      const fn = function( data, env, next ) {
        next( data, env );
      };
  ...
  function warnOnDeprecation( oldMethod, newMethod ) {
    return function() {
      if ( console.warn || console.log ) {
        const msg = "Warning, the " + oldMethod + " method has been deprecated. Please use " + newMethod + " instead.";
        if ( console.warn ) {
          console.warn( msg );
        } else {
          console.log( msg );
  ...
  const oldMethods = [ "withConstraint", "withConstraints", "withContext", "withDebounce", "withDelay", "withThrottle" ];
  const newMethods = [ "constraint", "constraints", "context", "debounce", "delay", "throttle" ];
  for ( let i = 0; i < 6; i++ ) {
  ...
  Monologue.prototype = {
    on: function( topic, callback ) {
      ...
      const subDef = new SubscriptionDefinition( topic, callback, self );
    ...
    off: function( topic, context ) {
      ...
      switch ( arguments.length ) {
        case 0:
          ...
          break;
        case 1:
          const type = Object.prototype.toString.call( topic ) === "[object String]" ? "topic" : topic instanceof SubscriptionDefinition ? "def" : "context";
          switch ( type ) {
            case "topic":
    ...
    emit: function( topic, data ) {
      const envelope = this.getEnvelope( topic, data );
      this._cache = this._cache || {};
      let cache = this._cache[ topic ];
      const invoker = function( subDef ) {
        subDef.invokeSubscriber( envelope.data, envelope );
      };
      if ( !cache ) {
        cache = this._cache[ topic ] = [];
        const cacherFn = getCacher( topic, cache, invoker );
