...
const AmqpStats = require('@indophi/node-amqp-stats')
...
function setupLogging(logging_levels = {}){
  let logging_type_keys = Object.keys(logging_levels);
  let log_config = [];

  logging_type_keys.forEach((logging_type_key) => {
    let log_filename = logging_levels[logging_type_key];
    let log_filepath = path.dirname(log_filename);

    try{
      fs.mkdirSync(log_filepath, { recursive: true });
    } catch(err){}

    let log = {
      level: logging_type_key,
      stream: fs.createWriteStream(log_filename),
      objectMode: true
    };
...
module.exports = function loopbackComponentMq(app, settings) {
  ...
  let log_config = setupLogging(settings.options.logging_levels);
  rabbit.log(log_config);

  //Habilita el logging de errores en la consola
